...
import { EngineDescriptor, GlobalManifest } from "./types";
...
function detectKind(filePath: string): EngineDescriptor["kind"] {
...
function detectSector(filePath: string): string | null {
  for (const hint of SECTOR_HINTS) {
    if (filePath.toLowerCase().includes(hint)) return hint;
  }
  return null;
}
